...
class CreepingDeathAttack:
    ...
    def inject(self, node):
        ...
        # Hook send - 修改这部分
        def hooked_send(this, socket, packet, host, port):
            target = (host, port)
            ...
            if should_drop(target):
                ...
                #     )
                return None
            return original_send(this, socket, packet, host, port)

        node.recv = hooked_recv
        node.send = hooked_send
