...
class Directory:
    ...
    def __init__(self, name: str, host: str, port: int):
        ...
        self.socket = self.socket_set(host, port)
    ...
    async def receive_loop(self):
        ...
        while True:
            result = await loop.run_in_executor(None, recv_udp, self.socket)
            if result:
                data, addr = result
                try:
    ...
    async def routing_dispatcher(self):
        while True:
            ...
            for info in self.registered_nodes.values():
                node_type = info.extra.get("type", "unknown")
                response.setdefault(node_type, []).append(asdict(info))
