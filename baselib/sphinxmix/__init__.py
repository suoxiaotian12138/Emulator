...
keys (derived using ``expon``).
    ...
    >>> from baselib.sphinxmix.SphinxClient import pki_entry, Nenc
    >>> pkiPriv = {}
    >>> pkiPub = {}
    >>> for i in range(10):
    ...     nid = i
...
first mix. Note both destination and message need to be ``bytes``.
    ...
    >>> from baselib.sphinxmix.SphinxClient import rand_subset, \\
...
by the sequence of mixes.
    ...
    >>> from baselib.sphinxmix.SphinxClient import PFdecode, Relay_flag, Dest_flag, Surb_flag, receive_forward
...
storing ``surbkeytuple`` keyed by the identifier ``surbid``:
    ...
    >>> from baselib.sphinxmix.SphinxClient import create_surb, package_surb
...
``dest``, where it may be decoded using the ``surbkeytuple``.
    ...
    >>> from baselib.sphinxmix.SphinxClient import receive_surb
...
parameter environment for the decoding of the message.
    ...
    >>> from baselib.sphinxmix.SphinxClient import pack_message, unpack_message
