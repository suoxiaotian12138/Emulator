...
from tools.Network_Management.DNSResolver import DNSResolver, DNSResolveError
...
from network_src.TorCore.circid_alloc import allocate_circid, validate_incoming_create_circid
from network_src.TorCore.Tor_Descriptor import TorDescriptor_build
from network_src.TorCore.Tor_Router import Tor_Router_simple
from network_src.TorCore.Tor_Crypt import NtorServerKeyAgreement
from network_src.TorCore.Tor_Socket import Tor_Socket
from network_src.TorCore.Tor_Scheduler import CircuitSendScheduler
...
from network_src.TorCore.Tor_Cell import CellCerts
...
class Tor_Node(Tor_base):
    ...
    def is_exit_node(self) -> bool:
        return any(str(flag).strip().lower() == "exit" for flag in self.flags)
    ...
    async def handle_cell(self, cell, sock: Tor_Socket):
        ...
        if isinstance(cell, CellVersions):
            ...
            incoming_circ_id = cell.circuit_id
            circuit = sock.channel.recv_map.get(incoming_circ_id) or self.circuit_list.get_by_id(incoming_circ_id)
            if circuit is None or not circuit.circuit_nodes:
                return
            if sock == circuit.circuit_nodes[0].sock:
            ...
            self._cancel_circuit_resolves(circuit.id)
    ...
    async def handle_cell_relay(self, cell, circuit, origin_cell, sock):
        ...
        if isinstance(cell, CellRelayExtend2):
            ...
            setattr(origin_cell, "_inner", cell)
            await self._forward_relay_cell(circuit, in_sock=sock, out_sock=out_sock, cell=origin_cell)
        elif isinstance(cell, CellRelayResolve):
            await self._handle_relay_resolve(
                cell=cell,
                circuit=circuit,
    ...
    async def _handle_relay_resolve(
            self,
            cell: CellRelayResolve,
            circuit,
            origin_cell: RelayedTorCell,
            in_sock: Tor_Socket,
    ):
        cid = circuit.id
        sid = origin_cell.stream_id

        # RESOLVE 必须绑定一个非零 stream ID。
        if sid == 0:
            self._ev(
                "resolve_invalid_stream",
                circ_id=cid,
                stream_id=sid,
                hostname=cell.hostname,
            )

            await self._close_circuit_protocol(
                circuit,
                in_sock,
            )
            return

        # 防御性检查。正常 onion routing 下，
        # 只有目标 Exit 才应识别到这个 relay command。
        if not self.is_exit_node():
            self._ev(
                "resolve_rejected_non_exit",
                circ_id=cid,
                stream_id=sid,
                hostname=cell.hostname,
            ...
            await self._send_resolved_error(
                circuit=circuit,
                out_sock=in_sock,
                stream_id=sid,
                transient=False,
        ...
        task = self._spawn_bg_task(
            self._resolve_and_reply(
                circuit=circuit,
                out_sock=in_sock,
                stream_id=sid,
                hostname=cell.hostname,
            ),
            name=f"resolve:{cid}:{sid}",
        )

        self._pending_resolves[key] = task

        def _cleanup(_task):
            current = self._pending_resolves.get(key)

            if current is _task:
                self._pending_resolves.pop(key, None)
    ...
    async def _resolve_and_reply(
        ...
        try:
            normalized = self._normalize_resolve_name(
                hostname
            )
            ...
            if isinstance(literal, ipaddress.IPv4Address):
                ip, ttl = str(literal), 3600
            else:
                ip, ttl = await asyncio.wait_for(
                    self.dns_solver.resolve_ipv4_with_ttl(normalized),
                    timeout=self.resolve_timeout,
                )

            answers = [
                (
                    CellRelayResolved.TYPE_IPV4,
                    ip,
                    ttl,
                )
            ]
    ...
    def _normalize_resolve_name(
        ...
        if "\x00" in hostname:
            raise ValueError(
                "hostname must not contain NUL"
            )

        # 第一阶段不支持 PTR。
        if hostname.lower().endswith(".in-addr.arpa"):
            raise ValueError(
                "reverse DNS is not supported yet"
            )

        # 直接传入 IPv4 literal 时可以立即回复，
        # 也可以交给下面专门处理。
        try:
        ...
        if isinstance(parsed, ipaddress.IPv6Address):
            raise ValueError(
                "IPv6 RESOLVE is not supported yet"
            )
        ...
        ascii_name = ascii_name.rstrip(".").lower()
    ...
    async def _send_resolved(
        ...
        resolved = CellRelayResolved(
            answers=answers,
            circuit_id=circuit.id,
        )

        relay = circuit.make_relay(
            inner_cell=resolved,
            relay_type=CellRelay,
            stream_id=stream_id,
        )
    ...
    async def _send_resolved_error(
            self,
            circuit,
            out_sock: Tor_Socket,
            stream_id: int,
        ...
        answer_type = (
            ...
            if transient
            else CellRelayResolved.TYPE_ERROR_NONTRANSIENT
        )
    ...
    def _cancel_circuit_resolves(
        self,
        circuit_id: int,
    ):
        keys = [
            key
            for key in self._pending_resolves
            if key[0] == circuit_id
        ]

        for key in keys:
            task = self._pending_resolves.pop(
                key,
                None,
            )
