...
from network_src.TorCore.Tor_Stream import StreamsList
from network_src.TorCore.Tor_Router import Tor_Router_simple
from network_src.TorCore.Tor_Crypt import NtorKeyAgreement

# from torpy.keyagreement import NtorKeyAgreement
from network_src.TorCore.Tor_Cell import *
from network_src.TorCore.circid_alloc import Channel, CircuitRef, CircuitState, allocate_circid
import logging
...
from network_src.TorCore.Tor_Cell import CellDestroy  # 若类名不同，替换为你项目里的 DESTROY cell
from network_src.TorCore.Tor_Window import TorWindow
...
PREBUILD_OPEN = 0                # keep 2 hot OPEN circuits
BUILD_TIMEOUT_S = 60
EXTEND_TIMEOUT_S = 30
...
class TorCircuit(CircuitRef):
    ...
    def allocate_request_stream_id(self) -> int:
        """Reserve a stream ID for a non-TCP relay request such as RESOLVE."""
        self.last_used = time.time()
        return self.streams.allocate_id()

    def release_request_stream_id(self, stream_id: int) -> None:
        self.streams.release_id(stream_id)
    ...
    def close_all_streams(self):
        # Keep this method synchronous because node/client teardown call sites do not await it.
...
class CircuitManager:
    ...
    def _unregister(self, circ_id: int):
        ...
        if s is not None:
            s.discard(circ_id)
            if not s:
                self.index.pop(m.isolation_key, None)
