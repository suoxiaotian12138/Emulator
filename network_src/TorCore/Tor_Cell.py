...
class CellRelayResolve(TorCell):
    """RELAY_RESOLVE: request a forward or reverse hostname lookup."""

    NUM = 11

    def __init__(self, hostname, circuit_id=0):
        super().__init__(circuit_id)
        if isinstance(hostname, bytes):
            hostname_bytes = hostname
            try:
                hostname_text = hostname.decode('ascii')
            except UnicodeDecodeError:
                hostname_text = hostname.decode('utf-8', errors='replace')
        else:
            hostname_text = str(hostname)
            hostname_bytes = hostname_text.encode('idna')

        if not hostname_bytes:
            raise ValueError("RESOLVE hostname must not be empty")
        if b'\x00' in hostname_bytes:
            raise ValueError("RESOLVE hostname must not contain NUL bytes")
        if len(hostname_bytes) + 1 > RelayedTorCell.MAX_PAYLOD_SIZE:
            raise ValueError("RESOLVE hostname is too long")

        self.hostname = hostname_text
        self._hostname_bytes = hostname_bytes

    def _serialize_payload(self):
        # Tor specifies a NUL-terminated hostname (or in-addr.arpa name).
        return self._hostname_bytes + b'\x00'
    ...
    def _deserialize_payload(payload: bytes, proto_version: int):
        try:
            ...
            raise ValueError("RESOLVE payload is missing its NUL terminator")

        hostname_bytes = payload[:end]
        if not hostname_bytes:
            raise ValueError("RESOLVE hostname must not be empty")
    ...
    def _args_str(self):
        return 'hostname = {!r}'.format(self.hostname)


class CellRelayResolved(TorCell):
    """RELAY_RESOLVED: return zero or more DNS lookup answers."""
    ...
    TYPE_IPV4 = 0x04
    TYPE_IPV6 = 0x06
    TYPE_ERROR_TRANSIENT = 0xF0
    TYPE_ERROR_NONTRANSIENT = 0xF1

    def __init__(self, answers, circuit_id=0):
        super().__init__(circuit_id)
        self.answers = list(answers)

    @classmethod
    def _encode_value(cls, answer_type, value):
        if isinstance(value, bytes):
            return value
        if answer_type == cls.TYPE_IPV4:
            return socket.inet_aton(value)
        if answer_type == cls.TYPE_IPV6:
            return socket.inet_pton(socket.AF_INET6, value)
        return str(value).encode('utf-8')

    @classmethod
    def _decode_value(cls, answer_type, value):
        if answer_type == cls.TYPE_IPV4 and len(value) == 4:
            return socket.inet_ntoa(value)
        if answer_type == cls.TYPE_IPV6 and len(value) == 16:
            return socket.inet_ntop(socket.AF_INET6, value)
        if answer_type == cls.TYPE_HOSTNAME:
    ...
    def _serialize_payload(self):
        ...
        for answer in self.answers:
            if len(answer) != 3:
                raise ValueError("Each RESOLVED answer must be (type, value, ttl)")

            answer_type, value, ttl = answer
            answer_type = int(answer_type)
            ttl = int(ttl)
            value_bytes = self._encode_value(answer_type, value)

            if not 0 <= answer_type <= 0xFF:
                raise ValueError("RESOLVED answer type must fit in one byte")
            if len(value_bytes) > 0xFF:
                raise ValueError("RESOLVED answer value is too long")
            if not 0 <= ttl <= 0xFFFFFFFF:
                raise ValueError("RESOLVED TTL must fit in four bytes")

            payload += struct.pack('!BB', answer_type, len(value_bytes))
            payload += value_bytes
            payload += struct.pack('!I', ttl)

        if len(payload) > RelayedTorCell.MAX_PAYLOD_SIZE:
            raise ValueError("RESOLVED answers exceed the relay payload limit")
        return bytes(payload)

    @classmethod
    def _deserialize_payload(cls, payload: bytes, proto_version: int):
        answers = []
        offset = 0

        while offset < len(payload):
            if len(payload) - offset < 2:
                raise ValueError("RESOLVED answer header is truncated")

            answer_type, value_len = struct.unpack_from('!BB', payload, offset)
            offset += 2
            required = value_len + 4
            if len(payload) - offset < required:
                raise ValueError("RESOLVED answer value or TTL is truncated")

            value_raw = payload[offset:offset + value_len]
            offset += value_len
            ttl = struct.unpack_from('!I', payload, offset)[0]
            offset += 4

            answers.append((answer_type, cls._decode_value(answer_type, value_raw), ttl))

        return {'answers': answers}

    def _args_str(self):
        return 'answers = {!r}'.format(self.answers)
