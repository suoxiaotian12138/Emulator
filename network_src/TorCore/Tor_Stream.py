...
class StreamsList:
    ...
    def __init__(self, circuit):
        ...
        self._next_stream_id = self.MIN_STREAM_ID
        self._circuit = circuit

    def allocate_id(self) -> int:
        """Allocate a non-zero stream ID unique within this circuit."""
        candidate = self._next_stream_id
        for _ in range(self.MAX_STREAM_ID):
            if candidate not in self._allocated_ids:
                self._allocated_ids.add(candidate)
                self._next_stream_id = candidate + 1
                if self._next_stream_id > self.MAX_STREAM_ID:
                    self._next_stream_id = self.MIN_STREAM_ID
                return candidate
            candidate += 1
            if candidate > self.MAX_STREAM_ID:
                candidate = self.MIN_STREAM_ID
        raise RuntimeError("No free stream IDs remain on this circuit")

    def release_id(self, stream_id: int) -> None:
        self._allocated_ids.discard(int(stream_id))

    def is_id_in_use(self, stream_id: int) -> bool:
        return int(stream_id) in self._allocated_ids

    def create_new(self):
        stream_id = self.allocate_id()
        stream = Tor_Stream(stream_id, self._circuit)
        self._stream_map[stream.id] = stream
        return stream

    def set_stream(self, stream_id, target_addr):
        stream_id = int(stream_id)
        if not self.MIN_STREAM_ID <= stream_id <= self.MAX_STREAM_ID:
            raise ValueError(f"stream_id out of range: {stream_id}")
        if stream_id in self._allocated_ids:
            raise ValueError(f"stream_id already in use: {stream_id}")
        self._allocated_ids.add(stream_id)
