from Routing.Node_verification_Tor import router_can_choose_node, nodelist_get_list, exit_nodelist_get_list
from temp import router_choose_random_node
from Routing.Circuit_Tor import Circuit, CpathBuildState
import os
...
crn_flags = jsonReader.get_tor_config_params("crnFlags")
...
class Node_choose():
    ...
    def onion_pick_cpath_exit(self, circ:Circuit, exit_ei=None):
        """
        为电路选择合适的路径长度并挑选出口路由器
        """
        state = circ.state
        ...
        if state.onehop_tunnel:
            ...
            path_len = new_route_len(circ.purpose, exit_ei, nodelist_get_list())
            if path_len < 1:
                raise ValueError("Invalid path length: must be at least 1")
            state.desired_path_len = path_len
        ...
        if exit_ei:
            ...
            node = choose_good_exit_server(circ, state.is_internal)
            if not node:
                raise ValueError("Failed to choose an exit server")
            state.chosen_exit = node
        return state.chosen_exit




def new_route_len(purpose, exit_ei, nodes):
    """
    计算新的路由长度。
    """
    if not nodes:
    ...
    num_acceptable_direct = count_acceptable_nodes(nodes, 1)
    num_acceptable_indirect = count_acceptable_nodes(nodes, 0)

    if num_acceptable_direct < 1 or num_acceptable_indirect < routelen - 1:
        raise ValueError(
            f"Not enough acceptable routers ({num_acceptable_direct}/{routelen} direct "
            f"and {num_acceptable_indirect}/{routelen} indirect routers suitable)"
        )

    return routelen


def route_len_for_purpose(purpose, exit_ei):
    """
    根据电路用途计算路由长度。
    :param exit_ei: extend_info_t 对象，出口节点信息（可选）
    :return: int，路由长度
    """
    routelen = circuit_params.DEFAULT_ROUTE_LEN

    # 使用vanguards的隐藏服务暂不实现

    if not exit_ei:
        return routelen

    if purpose in (
            circuit_params.CIRCUIT_PURPOSE_CONFLUX_UNLINKED,
            circuit_params.CIRCUIT_PURPOSE_TESTING
    ):
        known_purpose = True
    elif purpose in (
            circuit_params.CIRCUIT_PURPOSE_C_GENERAL,
            circuit_params.CIRCUIT_PURPOSE_C_HSDIR_GET,
            circuit_params.CIRCUIT_PURPOSE_S_HSDIR_POST,
            circuit_params.CIRCUIT_PURPOSE_C_INTRODUCING,
            circuit_params.CIRCUIT_PURPOSE_S_CONNECT_REND,
            circuit_params.CIRCUIT_PURPOSE_S_ESTABLISH_INTRO
    ):
        known_purpose = True
        routelen += 1
    else:
        routelen += 1  # 默认情况下增加一跳以确保安全

    return routelen


def count_acceptable_nodes(nodes, direct):
    """
    计算可接受的节点数量。
    :param nodes: smartlist_t，节点列表
    :param direct: int，是否需要直接连接（1 为是，0 为否）
    :return: int，可接受的节点数
    """
    num = 0
    flags = crn_flags.CRN_NEED_DESC
    if direct:
        flags |= crn_flags.CRN_DIRECT_CONN
...
def choose_good_exit_server(circ, is_internal):
    """
    选择合适的出口节点。
    Args: circ: flags: 路由選擇標誌，is_internal: 是否為內部電路
    """
    purpose = circ.purpose
    nodelist = exit_nodelist_get_list()
    if purpose in [circuit_params.CIRCUIT_PURPOSE_CONFLUX_UNLINKED, circuit_params.CIRCUIT_PURPOSE_C_GENERAL,
                   circuit_params.CIRCUIT_PURPOSE_HS_VANGUARDS, circuit_params.CIRCUIT_PURPOSE_S_HSDIR_POST,
                   circuit_params.CIRCUIT_PURPOSE_C_HSDIR_GET]:
        return router_choose_random_node(nodelist, circ, is_internal)
    # 隐藏服务电路暂时先不实现
    else:
        print(f"Unhandled purpose {purpose}")
        return None
...
def router_choose_random_node(nodelist, circ:Circuit, is_internal = False):
    """
...
if __name__ == '__main__':
    ...
    circ = Circuit(build_state=build_state)
    node_choose = Node_choose()
    result = node_choose.onion_pick_cpath_exit(circ)
    print(f"Result: {result}, Path length: {circ.state.desired_path_len}")