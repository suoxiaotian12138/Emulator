...
TARGET_PORT=8000 \
python -m example_test.senmantic\ test.semantic_runner
```
...
python -m example_test.senmantic\ test.semantic_log_analysis \
  "$TOR_LOG" "$TORBOX_LOG" \
...
```bash
python -m example_test.senmantic\ test.send\ me \
...
python -m example_test.senmantic\ test.state \
  --tor exp/semantic_logs/tor/multi_run_manifest.json \
  --torbox exp/semantic_logs/torbox/multi_run_manifest.json \
  --out semantic_outputs/plots \
